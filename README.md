...
官方 Lark/Feishu CLI 已安装，版本为 `1.0.76`。它能覆盖任务、云文档、消息、日历和会议能力。当前 CLI 尚未完成 OAuth，先把全局 npm bin 加入 PATH，再运行：
...
完成授权后，可将 CLI 的只读输出转换为 `data/context.json` 中的 `feishu.tasks`、`feishu.schedule`、`feishu.notes` 和 `feishu.messages`。Jessboard 不会把 OAuth 凭证写入前端。
...
- 完成官方 Lark CLI 的 OAuth 授权。
- 将全部可访问 Codex 会话和指定飞书项目写入同步快照。

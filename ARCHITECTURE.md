...
- `app.js`：维护本机任务、视图切换、专注计时、上下文状态、每日简报交互、开发分析和资讯渲染；资讯按市场、主题、重要度和新鲜度分页为独立流式报纸版面，并支持中文/英文即时切换。
